...
export async function enqueueInactiveProjectBatch(
  ...
  inactiveQueueWrite = operation.catch((err) => {
    logger.warn("failed to persist inactive-project tasks", { error: String(err) })
  })
  await operation
  return written ? ids : []
}
...
export async function cancelTask(taskId: string): Promise<void> {
  ...
  await saveQueue(currentProjectPath)
  logger.info("cancelled task and retained for restart", { sourcePath: task.sourcePath })
...
export async function cancelAllTasks(): Promise<number> {
  ...
  clearUsageLimitAutoResume()
  await saveQueue(currentProjectPath)
  logger.info("cancelled all tasks and retained", { count: cancelled })
  if (!hadProcessingTask) processNext(currentProjectId)
  return cancelled
}
...
export function pauseProcessing(): void {
  ...
  if (processingTasks.length > 0) {
    ...
    saveQueue(currentProjectPath).catch((err) => {
      logger.warn("failed to persist paused queue", { error: String(err) })
    })
  }
  logger.info("paused — in-flight task aborted; no new tasks will start")
}
...
export async function restoreQueue(
  ...
  if (pending > 0 || restored > 0) {
    logger.info("restored queue: pending paused for manual resume, failed, reset from interrupted", { pending, failed, restored })
...
function scheduleUsageLimitAutoResume(projectId: string): void {
  ...
  usageLimitResumeTimer = setTimeout(() => {
    ...
    if (currentProjectId !== projectId) return
    paused = false
    logger.info("auto-resuming after provider usage limit pause")
    processNext(projectId)
...
async function runTask(
  ...
  try {
    ...
    await saveQueue(projectPath)
    logger.info("task done", { sourcePath: task.sourcePath })
  } catch (err) {
    if (currentProjectId !== projectId || run.epoch !== queueEpoch) return
    const currentTask = queue.find((candidate) => candidate.id === task.id)
    if (cancelledInFlightTaskIds.delete(task.id) || currentTask?.status === "cancelled") {
    ...
    if (isUsageLimitError(message)) {
      ...
      logger.info("paused after provider usage limit; will retry automatically", {
        retryInMinutes: Math.round(USAGE_LIMIT_AUTO_RESUME_MS / 60000),
        error: message,
      })
      return
    }
    ...
    if (task.retryCount >= MAX_RETRIES) {
      task.status = "failed"
      logger.warn("task failed permanently", { sourcePath: task.sourcePath, retryCount: task.retryCount, error: message })
    } else {
      task.status = "pending"
      logger.warn("task error (will retry)", { sourcePath: task.sourcePath, retryCount: task.retryCount, maxRetries: MAX_RETRIES, error: message })
    }
    await saveQueue(projectPath)
  } finally {
    run.releaseCommitTurn()
...
async function startTask(projectId: string, task: IngestTask): Promise<boolean> {
  ...
  logger.info("processing task", {
    ...
    remaining: queue.filter((candidate) => candidate.projectId === projectId && candidate.status === "pending").length,
  })
