...
async function buildWikiGraphUncached(projectPath: string): Promise<WikiGraphResult> {
  ...
  for (const [sourceId, nodeData] of nodeMap) {
    for (const targetRaw of nodeData.links) {
      ...
      // then by page title via titleIndex (translated titles, zh-batch fix2)
      const targetId = resolveTarget(targetRaw, targetIndex, titleIndex)
...
export function resolveTarget(
  ...
  // (same second table as the server-side graph.rs resolve_wikilink; zh-batch fix2).
  if (titleIndex) {
