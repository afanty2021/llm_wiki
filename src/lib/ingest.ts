...
async function savedImagesFromMineruMarkdown(
  ...
  for (const relPath of refs) {
    ...
    try {
      ...
      logger.warn("failed to read cached MinerU image", {
        relPath,
...
async function appendIngestWarningLog(
  ...
  try {
    ...
    logger.warn("failed to write ingest warning log", {
      sourceIdentity,
      error: err instanceof Error ? err.message : String(err),
    })
  }
...
async function autoIngestImpl(
  ...
  if (isPdf && mineruCfg.enabled && mineruConfigured) {
    try {
      ...
      logger.debug("cached MinerU output", {
        fileName,
        chars: mineruResult.markdown.length,
        images: mineruSavedImages.length,
      })
    } catch (err) {
  ...
  if (useWikiStore.getState().sourceWatchConfig.persistExtractedMarkdown) {
    try {
      ...
      logger.warn("failed to keep parsed Markdown copy", {
        sourceIdentity,
        error: err instanceof Error ? err.message : String(err),
      })
    }
  ...
  if (
    ...
    logger.warn("Skipping cache save — write failures / truncated FILE blocks still missing", {
      sourceIdentity,
      writeFailures: hardFailures.length,
      truncatedMissing: unrecoveredTruncatedPaths.length,
    })
  }
...
async function executeIngestWritesImpl(
  ...
  for (const match of matches) {
    ...
    if (!isSafeIngestPath(relativePath) || isAppManagedAggregatePath(relativePath)) {
      logger.warn("executeIngestWrites rejected unsafe or app-managed path", { relativePath })
      continue
    }
