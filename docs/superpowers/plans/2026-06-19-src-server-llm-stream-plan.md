...
### Step 2: LlmConfig 加 timeout_secs

`src-server/src/services/llm.rs` 第 6 行的 `pub struct LlmConfig` 加字段：
...
/// 读 provider_type、endpoint、model、api_key_encrypted → 解密 key → 构造 impl。
pub async fn provider_for_project(
    ...
    .ok_or_else(|| AppError::ResourceNotFound(
        "LLM provider not configured for this project".into()
    ))?;
    ...
    .map_err(|e| AppError::InternalError(format!("decrypt api_key: {}", e)))?;

    let timeout = config.timeout_secs;

    match row.provider_type.as_deref() {
        Some("openai") => Ok(Box::new(OpenAiProvider::new(
            row.endpoint.unwrap_or_else(|| "https://api.openai.com/v1/chat/completions".into()),
            api_key,
            row.model.unwrap_or_else(|| "gpt-4o".into()),
            timeout,
        ))),
        Some("anthropic") => Ok(Box::new(AnthropicProvider::new(
            row.endpoint.unwrap_or_else(|| "https://api.anthropic.com/v1/messages".into()),
            api_key,
            row.model.unwrap_or_else(|| "claude-sonnet-4-20250514".into()),
            timeout,
        ))),
...
> **可能遇到的编译问题**：
> - `sqlx::query!` 宏需编译期 DATABASE_URL。若 CI 无 DB，改用运行时 `sqlx::query_as`（本 crate 已有宏使用模式，参考既有代码）。
> - `ProviderNotFound` variant 实际未被用（factory 对所有未知 type 返 ValidationError）。若 lint 报 `variant never constructed`，加 `#[allow(dead_code)]` 或移除该 variant（MVP 暂保留以备后续）。
> - `async_stream::stream!` 若编译报错，改用 `futures::stream::unfold` 手写（略冗长但无额外 dep）。
