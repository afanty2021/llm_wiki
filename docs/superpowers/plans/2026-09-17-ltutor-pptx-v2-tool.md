...
- `jszip` 从 devDependencies **升 dependencies**（评审 I-5，Task 2 运行时用；**改 package.json 后 `npm install` 再生 package-lock.json 并同 commit——ci.yml:62/:164 两处 `npm ci` 严格校验 lockfile 一致，不同步即 EUSAGE 必红**，计划评审 C2）。
...
- 模型路径：`LTUTOR_MEDIA__WHISPER_MODEL` 缺省 = **dist 产物（`dist/src/media-extract.js`）起四层 dirname 至 repo root** 再拼 `tools/transcriber/models/ggml-large-v3-turbo.bin`（计划评审 I-7：非「向上一级」）；解析封装纯函数+测试；**模型缺失 stat 预检走二进制缺失同款友好文案**。**唯一模型即 turbo**（设计 §三-1：~/.cache/whisper 是 Python .pt，勿读）。

### 2. 入参与安全

- schema：`{ wecom_userid（标准可选首参）, media_path（必填 string）, want（enum auto/transcript_only，缺省 auto）}`，顶层 `additionalProperties:false`。
- **允许根校验（D6）**：`path.resolve` 后必须以 `~/.hermes/cache/` 前缀（覆盖 images/documents/audio/video 入站缓存与自家 ltutor-media）；`..`/符号链接 resolve 后仍须在根内；**越界 → `{ok:false}` 正常文本返回（isError=false），勿抛 ToolArgumentError**（计划评审 C3 统一：media_path 由模型转写注记行、抄错完全可能，-32602 进熔断器 3 次即整服务器假故障 60s——training.ts:754 前例注记；v1 的 PptxFormatError→ToolArgumentError 先例仅适用 schema 已约束的形状错误）。execFile 全程 argv 数组无 shell。
...
- 幻觉过滤拷贝件（计划评审 I-5 权威清单，**照行号拷勿凭名忆**——whisper.ts 实测）：`HALLUCINATION_TOKENS`（whisper.ts:34）/`stripHallucinationSegments`（:41）/`analyzeWindowDegeneration`（:87）/`analyzeDegeneration`（:134）——**四个纯函数含窗级退化守门**（勿漏后两个）；音频提取参数 `audioArgs` 在 **audio.ts:19**（非 whisper.ts，`-vn -ac 1 -ar 16000 -c:a pcm_s16le` 实测符合）。**`runDegenerationGate` 非纯勿拷**（env 口子+console.warn 副作用，设计评审 §三-2）。一致性四用例防拷贝走样。
- **wav 先行是承重次序（计划评审 M10）**：先提 wav→whisper 用毕即删，再产 mp3/sheet——wav 是 28MB 大头且 whisper 是唯一消费者，先行+即删把磁盘占用窗口压到最短；实施时勿因「可并行」重排（并行化会把 wav 生存期拉长到 ffmpeg 组结束）。
...
4. handler 测试：身份注入/超帽引导不调管线/互斥忙返回/二进制缺失透传/成功无 MEDIA 行（fake 注入，同 v1 套路）；**路径越界负向断言「不抛 ToolArgumentError、返回 ok:false 文本」**（计划评审 C3）。

## 五、Task 4：SKILL.md + flow-pptx.md v2

- **SKILL.md**：description 补「视频/音频素材转课件（自动提取内容）」；快速路径行改「把这个视频/音频做成课件」→ 流程 10（骨架：`media_extract` 提取 → `vision_analyze` 读帧 sheet → `teacher_tutor_pptx` 出片 → MEDIA 回显）；§2 白名单 18→19 + extract 行（参数形状 + steer 中性提示）；**§12 桩一行更新（计划评审 I4）**：现桩括注「视频/音频转课件 v1 边界话术在流程文件内钉死」在 v2 下成假话——改为指针性描述（流程 10 含多素材提取）；**白名单 pptx 行参数摘要顺手补 `image`/`audio_path`**（计划评审 M11）。
- **flow-pptx.md v2 重写**：四锚头保留；正文=输入矩阵（视频/音频→先 extract；图片→vision_analyze 直读；文本→直构；库内→search/read_file v1 路）→ 提取启动回复话术（steer 勘误后中性版：「素材处理约 1-2 分钟请稍候；期间补发消息不会打断处理，但建议素材发齐后再开始」）→ 帧读（vision_analyze 读 sheet 单次调用）→ 构造（image/audio 嵌入指引：音频嵌封面可选）→ MEDIA 回显 → 失败回退（环境性勿重试刷屏，文字版大纲兜底）。**边界话术**：>15 分钟或 >20MB 引导截片段或入库（库内素材走 v1 转写页路）。**设计三处点名补入 flow 清单（计划评审 M9）**：①无音轨视频/纯音频分支说明（视频无音轨→仅帧路、音频无帧→仅转写）；②竖屏视频 contact sheet 可读性回退（3×3 不达→2×2 四帧，真发目检触发）；③doc-cache 24h 清理的隔天重做话术（重发素材即可，课件本身已自包含）。
- **busy 表述（勘误后）**：profile `busy_input_mode=steer`——补发不打断在飞提取；flow/SKILL 只需上述中性提示，不再出现「会中断处理」字样（原 N1 吸收段随勘误作废）。
- 双份 cp 部署留 Task 5（重启先于 cp，v1 I-4 次序不变）。

## 六、Task 5-6：部署与验收

1. 部署序（v1 定例；重启措辞按计划评审 I-6 更正）：`npm --prefix mcp-server test` 全绿 → `npm run mcp:build` → 网关重启 = **`gateway run --replace`（本批无 launchd 改动，勿 bootout——plist 本身就是 `--replace` 形态且有 5 分钟 monitor 保活，bootout 反而与 monitor 竞态；v1 两次 bootout 首试均撞 exit 5 即此因）**；**避周日 19:00-20:00 窗**、重启前查在飞教师回合 → SKILL.md+flow-pptx.md 双份 cp + md5 → stdio 握手验**17 工具**（15 teacher_tutor + 2 llm_wiki）；部署窗顺手更正 profile config 注释 16→17。
2. **真发验收五场景**（用户手动，设计 §九-3）：①≤3 分钟视频→两步工具调用+帧内容进页；②语音→转写驱动；③2-3 张教材照片→图片页；④视频课件+音频嵌封面→PowerPoint 点喇叭可播；⑤**提取运行中补发一张图片→验证 steer：提取照常完成、补发在工具批次结束后同轮到达**。
...
- **handler**：extract 四用例（含**路径越界不抛 ToolArgumentError 负向断言**）+ pptx 嵌入路径用例；**transcript_truncated 截断标记用例**（计划评审 M8）；**计数钉 16→17 三处同步**。
...
| jszip 移动致 CI 红 | package.json + lockfile 同 commit 再生（ci.yml npm ci 严格校验，计划评审 C2）；CI frontend-test 绿 = Task 1 完成门槛 |
| 提取中教师补发素材 | profile busy_input_mode=steer（勘误后设计 D4）：不打断在飞提取、补发同轮到达；flow 中性提示 + 场景⑤验证 |
| 路径越界误进熔断器 | 统一 `{ok:false}` 文本返回 + handler 负向断言（计划评审 C3） |
