...
### 5.5 体积预期（r2 修正算术，评审 I1）

核心 ≈ **19.2-19.8KB**（27,590 − 外移 9,140 + 桩 ~600 + 四行骨架保留净增 ~416 + §0-4/§2 改句 ~200），**降幅 ~28-30%**。验收线：**≤20KB**。质量收益：多数回合（答疑/清单/完成）不驮 9.1KB 无关流程；§11/§7 只在对应上下文出现；压缩后两跳指针链自愈（§1.5）。
