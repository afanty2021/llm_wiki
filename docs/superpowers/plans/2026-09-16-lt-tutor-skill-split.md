...
- ~~闭环复审残余四 Minor 之测试两条：新用例纯负向钉扎（删消毒行不红，401 归因行无保护）+ 宽守卫非 ImportError 分支无用例~~ ✅ 源仓 `fd70bd1` 测试补强：前者补正向断言（「dotenv fallback lookup failed for 变量 (RuntimeError)」归因行必须在 caplog 中存在——删 warning 本身即红，「无钥」无法靠静默删日志蒙混）+ helper 消息文本排除断言；后者新增 `test_import_explosion_non_importerror_degrades`（import 期非 ImportError 同样类名级 debug 降级 None）。双钉变异实跑证真（删 warning 行/窄化回 ImportError 各自变红，恢复后绿、__init__.py md5 复原）；套件 12→13 用例，插件全量 83 passed, 2 skipped。另两条 Minor 无需改码：daemon 前提 pid 过时系执行会话简报层问题、两仓文档均无此前提（复审报告已纠正，免重启结论不变）；记忆 description 陈旧已更新。
