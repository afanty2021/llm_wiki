...
1. **取材**：先 `llm_wiki_search` + `llm_wiki_read_file` 读库内真实课文——导图内容必须来自真实材料，**不编造课文外内容**。
2. **构造大纲**：从材料提炼层级结构，调 `teacher_tutor_mindmap`：`title`（中心主题，≤60 字符）、`root.children`（分支树，每节点 `{label, children?}`，标签 ≤40 字符，整图 ≤5 层 ≤60 节点）。超限会被拒——按章节拆成多张，或与老师确认精简。
