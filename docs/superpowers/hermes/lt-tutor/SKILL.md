...
5. **推荐落地（2026-09-07 补）**：作答中推荐了 **≥2 个具体可学资源**（视频/页面）→ 顺手 `teacher_tutor_plan_create` 建小清单（`origin:"chat"`；**多视频时**视频优先规则同流程 3）并附整单链接——**推荐即清单**：跨会话可寻址（教师隔天说"把那个视频加进清单"时直接对上号），周报进度管线也吃得到。仅随口提及单个资源则不必建单。
