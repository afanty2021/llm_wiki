...
- `delete_page` 路由（`routes/pages.rs:253-273`）含 embedding 清理 → `delete` action 复用同款 SQL。**注意**：`graph.rs::build_graph` 缓存键是 `(project_id, MAX(updated_at))`，DELETE 不改剩余行的 `updated_at`，故删除非最新页时缓存键不变、命中旧图（pre-existing，`delete_page` 路由同款问题）—— Phase B 的 `delete` executor 额外调 `graph::invalidate_project_cache(project_id)` 显式清缓存（见 §9.3）。
...
| `services/graph.rs` | 新增 `pub fn invalidate_project_cache(project_id)`（delete executor 清缓存用；DELETE 不触发时间戳失效） | 改 |
...
// dedicated review stage（best-effort：LLM 失败只记 warning，不阻断摄取 —— §8.3/§10；
// 不得用 `?`，否则 dedicated 超时/限流会让 process_source_path 返回 Err、丢弃该源全部已算 wiki 页）
match crate::services::llm_stream::provider_for_project(state, project_id).await {
    Ok(provider) => match crate::services::review::run_dedicated_review_stage(
        state, project_id, source_path, &text, &step1_result, &llm_output, &*provider).await {
...
pub async fn run_dedicated_review_stage(
    ...
    let opts = ChatOpts { model: provider.model_name().to_string(), temperature: 0.4, max_tokens: 8000,
...
```
> `provider` 由调用方（`process_source_path`）经 `provider_for_project` 取得后注入 —— 这样 dedicated stage 可用 FakeProvider 单测（见计划 Task 4）。
...
| delete | `pages.rs::delete_page` 同款 SQL + `embedding::delete_embedding` + `graph::invalidate_project_cache` | 删 wiki_pages 行 + 清向量 + **显式清 graph 缓存**（DELETE 不改剩余行 updated_at，时间戳失效对 delete 不成立） |
