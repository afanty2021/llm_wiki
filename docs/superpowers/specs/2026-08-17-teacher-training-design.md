...
- 内容源：`~/Github/L T师训 2024-2025` + `~/Github/L T师训 2024-2025（HEVC）`（两目录合计约 1500 个媒体文件，仅存 HEVC 目录约 440-462 个；权威清单由 M0 对账产出）
...
### 3.1 M0 内容对账（先于一切转写）
- 扫描两目录（含 HEVC 独有约 440-462 个、跨目录精确同名仅 3 对——M0 归一化配对定权威；排除 `__MACOSX`），ffprobe 逐个：容器/视频编码/音频编码/时长
- **分桶（两桶，含音频维度）**：
...
- v1 事实修正存档：mp3↔mp4 归一化同名 0 对；HEVC 目录纳入；首批专栏 = 教学新知班级管理专栏（**视频主体在 HEVC 目录：44 个 mp4；主库同目录仅 4 个无扩展名文件**——历轮"36 个"系错误匹配口径，M0 对账定权威值）
