...
- **D4 同步单调用，不做任务队列**：预算 ≤280s ≤300s MCP 硬顶（分段表见 §五-3）。**busy 行为（实施计划评审 C1 勘误——设计评审 C3 只查主 config:243 interrupt 漏了 profile 覆盖）**：lt-tutor profile `display.busy_input_mode: steer`（config:87，2026-09-15 vision 转写事故 P0 修复）——**busy 期间新消息（含媒体类补发）不打断在飞工具调用**，排队到工具批次结束后同轮投递（park-and-merge，模型同见工具结果+新消息）；`/stop` 仍可硬停。教师提取中补发照片不丢不重。配套=flow 中性提示话术（§七）+ 验收场景⑤验证 steer（§九）。
...
- **flow 话术补 busy 提示（评审 C1 勘误后中性化）**：提取启动回复中告知「素材处理约 1-2 分钟请稍候；期间补发消息不会打断处理，但建议素材发齐后再开始」。
...
3. **真发验收矩阵**（用户手动，五场景）：①发一段 ≤3 分钟视频「做成课件」→ 两步工具调用 + 文件到达 + 帧内容进页；②发一段语音 → 转写驱动课件；③发 2-3 张教材照片 → 图片页课件；④复现参照会话形态——视频课件 + 音频嵌封面，下载后 PowerPoint 点封面喇叭可播；**⑤提取运行中教师补发一张图片（评审 C1）→ 验证 steer：提取不被打断照常完成、补发消息在工具批次结束后同轮到达**。
...
| 提取中教师补发素材 | profile busy_input_mode=steer（勘误后 D4）：不打断在飞提取，补发同轮到达；flow 中性提示话术 + 场景⑤验证 |
