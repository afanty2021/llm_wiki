...
|------|------|------|
| A | console 迁移 + 采样 | ✅ 已完成 |
| **B（本文档）** | read_log_file 命令 + 应用内日志查看器 | ✅ 已实施 |
| C | JSONL 结构化查询 | ❌ **关闭**（被批次 B 吸收） |

> **批次 C 关闭说明**：原计划的"JSONL 结构化查询"核心价值（按 trace_id、关键字、级别检索日志）已由批次 B 的查看器完整覆盖（LogsSection 的 level toggle + keyword 搜索 + trace_id 过滤即结构化查询的 UI 形态）。后续若需更复杂的多字段 AND/OR 查询或聚合统计，再单独立项。
