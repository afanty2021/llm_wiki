...
### Phase 2 — 向量库调优 ✅ 已完成（2026-06-26）
- 实施计划：`docs/superpowers/plans/2026-06-26-layer6-phase2-vector-tuning.md`
- 分支：`feat/layer6-phase2-vector-tuning`
- migration 011（chunk 列 + 维度收敛 1024）+ chunk 切分 + chunk 级 VectorStore（DELETE+INSERT）+ §5.4 三层聚合 + HNSW ef_search 事务
- embed_batch 指数退避重试；LLM rerank 接入 hybrid_search（失败 fallback RRF）
- **验收**：golden set 召回（alice 相关页 top-2 + 分数 > 无关页）；现有 search/embedding 测试绿；行为对存量数据向后兼容（chunk_index=0、chunk_text NULL 由 COALESCE 兜底）

### Phase 3 — 队列可靠性 ✅ 已完成（2026-06-27）
- 设计 spec：`docs/superpowers/specs/2026-06-26-layer6-phase3-queue-reliability-design.md`
- 实施计划：`docs/superpowers/plans/2026-06-26-layer6-phase3-queue-reliability.md`
- 分支：`feat/layer6-phase3-queue-reliability`（off `feat/layer6-phase2-vector-tuning`，8 commits + 本 docs 提交）
- migration 012（retry_count/max_retries/cancel_requested/lease_expires_at/item_states + status 加宽 VARCHAR(40)）
- 取消（协作式只停不清；pipeline 3 处 check_cancel 检查点 + Err(Cancelled) 短路）/ 重试（瞬态分类 is_transient_job_err + 自动退避 backoff_delay + 手动重置 retry_count=0）/ 部分失败隔离（worker 三态标记 succeeded/succeeded_with_warnings/failed；all-failed 用本地 done_this_run 计数修正既存 updated_reserved.is_empty() 恒假 bug）/ SSE（AppState broadcast::channel(64) + JobEvent + GET /stream，subscribe-before-snapshot 闭合快任务终态丢失窗口）
- 新 routes：POST `/ingest/jobs/:id/cancel`、POST `/ingest/jobs/:id/retry`、GET `/ingest/jobs/:id/stream`（tokio-stream `sync` feature 启用 BroadcastStream 适配）
- **验收**：180 lib 单测绿；集成测编译通过；cancel + manual-retry-reset 两个 `#[ignore]` 集成测通过；lifecycle 新字段（retry_count/cancel_requested）断言通过。2 个预存失败/抖动集成测（`ingest_queue`/`research`，需 PG+Redis）经 base 对照非回归。已知窄边：cancel 中途 → manual_retry → 剩余 source 全失败 时 done_this_run==0 会判 all-failed（self-healing + 只停不清，数据不丢）。
