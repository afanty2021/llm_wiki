...
pub enum ParseError {
    ...
    EncodingError { filename: String, encoding: String },  // 非 UTF-8 编码（如 GBK/Latin-1）
    PdfiumError(String),
    Io(String),
    CorruptFile(String),
}
...
pub struct ChatOpts {
    ...
    pub timeout_secs: Option<u64>,  // 默认来自 config，LLM 调用每请求超时
}
...
// 进度更新(worker/pipeline 用) — 三个 helper，位于 ingest_queue.rs
pub async fn update_job_stage(state: &AppState, job_id: Uuid, stage: &str, progress: i32) -> Result<(), AppError>;
pub async fn mark_job_failed(state: &AppState, job_id: Uuid, error: &str) -> Result<(), AppError>;
pub async fn mark_job_succeeded(state: &AppState, job_id: Uuid, result: &IngestJobResult) -> Result<(), AppError>;

// 启动 worker(server main 调) — ingest_worker.rs: worker loop + 恢复逻辑，内部调上方进度 helper
pub fn spawn_worker(state: AppState);
```

**文件职责澄清**：`ingest_queue.rs` — 数据模型 + 入队/查询/进度更新 4 个 helper；`ingest_worker.rs` — 循环调度(spawn_worker → worker_loop) + 重启恢复(扫 pending/running)，内部调 queue 的进度 helper 和 pipeline 编排。
...
pub struct IngestJobResult {
    ...
    pub warnings: Vec<String>,           // 多文件 job 的累积警告(部分成功信息)，为空即全成功
}

/// team_id 从 projects 表查得（因 IngestJob 只含 project_id）。
/// run_ingest_job 内部自行查 team_id 构造 storage base URL。
...
for each source_path in job.source_paths {
    ① 读 storage 源文件(state.storage_path, team_id 从 projects 查)
    ② 解析(A::parse_bytes) → ParsedDoc{text, images}
    ③ 存图片到 storage/media/{project_id}/ → ③' 替换 text 里图片引用路径
       原始相对路径(page3_image1.png)→ media/{project_id}/page3_image1.png(LLM 可见)
    ④ 长文档分块：
        if text 估算 token > context_budget -> 按段落边界拆 chunk
        -> 每 chunk Step1 分析 -> digest 合并 -> Step2 生成(同桌面策略)
    ⑤ 检查 ingested_files 表:查(project_id, original_path) → 比对 content_hash(SHA-256) + file_size →
        无变化跳过；变化 UPDATE 6 字段(original_path, content_hash, file_type, file_size, ingested_at)
    ⑥ 检查缓存：sha256(text) 在 redis ingest:cache:{hash} → 复用分析结果，跳过 Step1
    ⑦ Step1 LLM(B::stream_chat) → 结构化 JSON(实体/概念/连接/矛盾)
    ⑧ 缓存 step1 结果到 redis ingest:cache:{sha256}(TTL: 7 天，跨 project 复用)
    ⑨ Step2 LLM(B::stream_chat) → 多个 FILE block → parse_file_blocks 解析
    ⑩ 写 wiki_pages：逐 FILE block 用 ON CONFLICT(project_id,path) DO UPDATE 幂等写入
       (单个 page 失败不阻断 job——捕获错误写 warnings，继续处理余下 pages)
    ⑪ 更新进度(update_job_stage)
}
⑫ 重建 reserved pages(见下)——失败标记 job.failed(已写的 pages 保留，幂等可重试)
⑬ 标记 job.succeeded(含 warnings 若有部分失败)
```

**事务策略**：普通 page 写入用 ON CONFLICT upsert(不包大事务，幂等)。reserved pages 重建在同事务内(SELECT FOR UPDATE + 写回)。因 upsert 幂等，job 重试已写页面被覆盖而非重复。
...
**与现有模式对齐**：ingest endpoints（`/:pid/ingest`、`/:pid/ingest/jobs`）同 pages 一样，通过 `.merge()` 合入 `project_routes()`，而不是在 `routes/mod.rs` 对同一前缀做两次 `.nest()`。

```rust
// src/routes/projects.rs —— 现有 project_routes() 加一行 merge
pub fn project_routes() -> Router<AppState> {
    Router::new()
        .route("/:id", axum::routing::post(create_project))
        ...
        .merge(pages::pages_routes())
        .merge(ingest::ingest_routes())   // ← 新增
}

// src/routes/ingest.rs
pub fn ingest_routes() -> axum::Router<AppState> {
    axum::Router::new()
        .route("/:id/ingest",      axum::routing::post(create_ingest_job))   // project-scoped
        .route("/:id/ingest/jobs", axum::routing::get(list_ingest_jobs))
}

// GET /ingest/jobs/:id 不绑 project → 独立挂在 routes/mod.rs
pub fn global_ingest_routes() -> axum::Router<AppState> {
    axum::Router::new()
        .route("/api/v1/ingest/jobs/:id", axum::routing::get(get_job_status))
}
```
因为 `project_routes()` 已有 `/:id` 单段路由，而 `ingest_routes()` 里的 `/:id/ingest` 是两段，axum 优先匹配更具体的路径——两者不冲突。
...
| 文件 | 职责 | 改动 |
|------|------|------|
| `Cargo.toml`(根, **新建**) | workspace([workspace] members = ["src-server","crates/llm-wiki-parser"])——当前无 workspace,创建 crate 需先建 | Create |
| `crates/llm-wiki-parser/Cargo.toml` | A 解析 crate manifest | Create |
| `crates/llm-wiki-parser/src/lib.rs` + `parser/*.rs` | A 核心逻辑 | Create |
| `src-server/Cargo.toml` | 退出 workspace member + `reqwest` 依赖(LLM 流式客户端,当前未含) | Modify |
| `src-server/src/services/llm_stream.rs` | B LLM 流式客户端 | Create |
| `src-server/src/services/ingest_queue.rs` | C 数据模型 + 入队/查询/进度更新 helper | Create |
| `src-server/src/services/ingest_worker.rs` | C worker loop(spawn_worker) + 重启恢复 | Create |
