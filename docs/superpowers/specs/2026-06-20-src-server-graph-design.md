...
GET /api/v1/graph/:project_id
  ...
  7. 社区 info：cohesion = intra_edges / possible_edges（possible_edges = n>1 ? n(n-1)/2 : 1；
       **n=1 → cohesion=0（防除零 NaN）**）；topNodes = 按 linkCount top5；重编号 0,1,2...
...
fn resolve_wikilink(raw: &str, path_set: &HashSet<String>, stem_to_path: &HashMap<String,String>) -> Option<String>;
```
